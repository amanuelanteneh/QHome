...
void bluetoothLabel::deviceDiscovered(const QBluetoothDeviceInfo& device) {
	if (device.minorDeviceClass() == QBluetoothDeviceInfo::Headphones || 
		device.minorDeviceClass() == QBluetoothDeviceInfo::Loudspeaker) { //check if headphones or speaker
...
void bluetoothLabel::finishedScan() {
	...
	showMessage("Select the device you want to add then close the window.",
	"img/bluetoothImage", "Add BT Device Info");
...
void bluetoothLabel::deviceDisconnected() {
	...
	this->setPixmap(labelPic->scaled(wdt, hgt, Qt::KeepAspectRatio));	
	device->disconnectFromService(); //this is in case user disconnects manually like from settings or by turning off headphones
